...
if($admin->get_post('question') == '' AND $admin->get_post('answer') == '') {
	$admin->print_error($MESSAGE['GENERIC']['FILL_IN_ALL'], WB_URL.'/modules/rbtoggle/modify_question.php?page_id='.$page_id.'&section_id='.$section_id.'&question_id='.$question_id);
} else {
...
// Update row
$database->query("UPDATE ".TABLE_PREFIX."mod_rbtoggle_questions SET question='$question', answer='$answer', cat_id='$category' WHERE question_id='$question_id'");
